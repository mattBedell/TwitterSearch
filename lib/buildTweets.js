module.exports = function myTools(){
  const buildTweets = (tweetObj) => {
    ...
    tweetObj.forEach( (tEntries) => {
      let stripText = tEntries.text.replace(/(?:https?|ftp):\/\/[\n\S]+/g, ''); //REGEX SOURCE: http://stackoverflow.com/questions/23571013/how-to-remove-url-from-a-string-completely-in-javascript
       stripText = stripText.replace(/#([^\\s]*)/g, '');//REGEX SOURCE: http://stackoverflow.com/questions/3514357/strip-hashtags-from-string-using-javascript
